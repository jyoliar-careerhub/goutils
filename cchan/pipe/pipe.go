...
func Transform[INPUT any, OUTPUT any, ERROR error](ctx context.Context, inputChan <-chan INPUT, errChan chan<- ERROR, bufferSize *int, action func(INPUT) (OUTPUT, ERROR)) <-chan OUTPUT {
	bfs := 0
	if bufferSize != nil {
		bfs = *bufferSize
	}
...
func Pipeline2[INPUT any, M1 any, OUTPUT any, ERROR error](ctx context.Context, inputChan <-chan INPUT, errChan chan<- ERROR,
	step1 Step[INPUT, M1, ERROR],
	step2 Step[M1, OUTPUT, ERROR],
) <-chan OUTPUT {
	pipeChan := Transform(ctx, inputChan, errChan, step1.BufferSize, step1.Action)

	return Transform(ctx, pipeChan, errChan, step2.BufferSize, step2.Action)
}

func Pipeline3[INPUT any, M1 any, M2 any, OUTPUT any, ERROR error](
	ctx context.Context,
	inputChan <-chan INPUT, errChan chan<- ERROR,
	step1 Step[INPUT, M1, ERROR],
	step2 Step[M1, M2, ERROR],
	step3 Step[M2, OUTPUT, ERROR],
) <-chan OUTPUT {
	pipeChan := Pipeline2(ctx, inputChan, errChan, step1, step2)
	return Transform(ctx, pipeChan, errChan, step3.BufferSize, step3.Action)
}
...
func Pipeline4[INPUT any, M1 any, M2 any, M3 any, OUTPUT any, ERROR error](
	...
	pipeChan := Pipeline3(ctx, inputChan, errChan, step1, step2, step3)
	return Transform(ctx, pipeChan, errChan, step4.BufferSize, step4.Action)
}
